    ...
    return (
        <>
            <div className={`h-fit w-full peer relative overflow-hidden cursor-pointer group active:opacity-60 active:translate-y-3 active:scale-[0.98] active:rotate-[2deg] hover:z-10 hover:shadow-lg item`} style={{ background: `${avg}` }} onClick={viewHandler}>
                ...
                <span className="absolute group-hover:top-4 group-hover:right-4 top-3 right-3 px-2 lowercase py-1 bg-white sm:text-sm text-xs rounded-md font-bold z-30 sm:max-w-[20rem] max-w-[10rem] text-ellipsis whitespace-nowrap overflow-hidden border border-black/25 shadow-lg shadow-black/10">@{photographer}</span>
            ...
            {imgAlt && <p className="font-semibold md:text-sm text-xs text-center mb-3 mx-6 shadow shadow-transparent peer-hover:opacity-100 opacity-50 hover:shadow-black/10 text-black px-6 line-clamp-1 py-1 rounded-3xl border border-black/10 hover:border-black/25 bg-slate-100">{imgAlt}</p>}
