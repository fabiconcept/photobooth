...
export default function GridItem({ imgSrc, imgAlt, avg, photographer, photographer_url, contextObj }) {
    ...
    return (
        <>
            <button title={imgAlt} className={`h-fit w-full peer relative overflow-hidden cursor-pointer group active:opacity-60 active:translate-y-3 active:scale-[0.98] active:rotate-[2deg] hover:z-10 hover:shadow-lg item`} style={{ background: `${avg}` }} onClick={viewHandler}>
                <Image
                    width={600}
                    height={800}
