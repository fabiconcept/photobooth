"use client"
import { FaDownload, FaEyeDropper, FaUserAstronaut } from "react-icons/fa6";
...
export default function PopModal({imgSrc, imgAlt, photographer, avg, photographerUrl, status}) {
        ...
        <div className={`popModal ${modalShowing ? "in" : "out"}`}>
            <div className="modal">
                <div className="img">
                    ...
                    <Image height={800} width={600} src={imgSrc} alt={imgAlt} />
                </div>
                <section className="w-full p-5 rounded-b-lg bg-white flex-1 flex justify-between items-center">
                    ...
                    <span className="flex gap-4 items-center cursor-grab hover:scale-105 active:scale-95" onClick={()=>copyToClipboard(avg)}>
                        ...
                        <div className={`bg-[${avg}] color text-xs flex items-center justify-center`}>{avg}</div>
                    </span>
